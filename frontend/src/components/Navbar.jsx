...
const Navbar = ({ mode, toggleMode }) => {
  ...
  return (
    <>
      ...
      {/* Mobile Sidebar */}
      {mobileMenuOpen && <div className="p-4 flex flex-col h-full">
        ...
        <div className="flex flex-col space-y-4 flex-grow">
          {currentUser && (
            <Link
              to="/global"
              className="flex items-center p-3 text-gray-600 hover:text-gray-900 hover:bg-gray-100 rounded-lg transition-colors font-bold text-xl"
              onClick={() => setMobileMenuOpen(false)}
            >
              <svg className="w-5 h-5 mr-3 text-emerald-600" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" d="M12 4v16m8-8H4" />
              </svg>
              Global<span className={mode === 'dark' ? 'text-emerald-600' : 'text-emerald-600'}> Section</span>
            </Link>
          )}
          ...
          <motion.button
            ...
            onClick={toggleMode}
            className="flex items-center cursor-pointer p-3 text-gray-600 hover:text-gray-900 hover:bg-gray-100 rounded-lg transition-colors"
          >
            <svg className="w-5 h-5 mr-3" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
              {mode === "light" ? (
          ...
          <button
            ...
            className="flex items-center p-3 cursor-pointer text-gray-600 hover:text-gray-900 hover:bg-gray-100 rounded-lg transition-colors"
          >
            <svg className="w-5 h-5 mr-3" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
        ...
        {currentUser && <div className="mt-auto pt-4 border-t border-gray-200">
          <Link
            ...
            onClick={() => setMobileMenuOpen(false)}
          >
            <div className={`w-8 h-8 flex items-center justify-center rounded-full font-medium mr-3 ${mode === 'dark'
      ...
      <nav className={`bg-white/80 backdrop-blur-sm border-b border-gray-100 px-4 py-3 w-full ${mode === 'dark' ? 'dark:bg-gray-800/80 dark:border-gray-700' : ''}`}>
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <Link to="/" className="flex items-center text-emerald-600 hover:text-emerald-700 transition-colors">
            ...
            <span className="font-bold text-xl ml-2">
              Quicker<span className={mode === 'dark' ? 'text-gray-800' : 'text-gray-800'}>Notes</span>
            </span>
          </Link>
          {currentUser && !mobileMenuOpen && (
            ...
            >
              <Link
                to="/global"
                className="font-bold text-gray-700 text-xl relative group"
                style={{ paddingBottom: "4px" }}
              >
                Global
                <span className={mode === 'dark' ? 'text-emerald-600' : 'text-emerald-600'}>Section</span>
                <motion.div
                  layoutId="underline"
          ...
          {currentUser && (
            <div className="flex items-center space-x-2 sm:space-x-3">
              ...
              {currentUser && <Link to={`/profile?id=${currentUser._id}`} className="flex-shrink-0">
                <div className={`w-8 h-8 flex items-center justify-center rounded-full font-medium transition-colors ${mode === 'dark'
