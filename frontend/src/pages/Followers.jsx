...
import { FiUserPlus, FiUserMinus, FiUser, FiUsers } from 'react-icons/fi';
      ...
      <div className="container mx-auto px-4 py-8 max-w-6xl">
        ...
        >
          <div className="flex items-center gap-3">
            <FiUsers className={`text-3xl ${mode === 'dark' ? 'text-green-400' : 'text-green-600'}`} />
            <div>
        ...
        {loading ? (
          ...
          <div className={`text-center py-12 rounded-lg ${mode === 'dark' ? 'bg-gray-800' : 'bg-white shadow-sm'}`}>
            ...
            <p className="text-lg mt-4">No followers yet</p>
            <p className="text-sm opacity-75 mt-2">
              {id === user?._id ? "You don't have any followers yet" : "This user doesn't have any followers"}
            </p>
          </div>
        ) : (
          <div className="grid gap-4 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
            <AnimatePresence>
              {followers.map((follower) => {
                const isCurrentlyFollowing = myFollowingIds.includes(follower._id);
                const isCurrentUser = follower._id === user?._id;
                ...
                return (
                  <motion.div
                    ...
                    whileHover={{ y: -3 }}
                  >
                    <Link to={`/profile?id=${follower._id}`} className="block">
                      <div className={`rounded-xl p-4 transition-all ${mode === 'dark' ? 'bg-gray-800 hover:bg-gray-700' : 'bg-white hover:bg-gray-50 shadow-sm hover:shadow-md'}`}>
                        <div className="flex items-center gap-4">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-sm opacity-75 truncate">{follower.email}</p>
                          </div>
                          {!isCurrentUser && (
                            <motion.button
                              onClick={(e) => handleFollow(follower._id, e)}
