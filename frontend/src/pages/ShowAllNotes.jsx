...
//   let selectedPalete;
//   useEffect(() => {
//     async function changePalete(){
//       const response = await axios.get("http://localhost:3000/auth/palette/"+currentUser._id)
//       selectedPalete = response.data.palette;
//       console.log(selectedPalete);
//     }
//     changePalete();
//   })
            ...
            {filteredNotes.map((note) => (
              ...
              >
                ...
                <div className="mt-4">
                  ...
                  {isOpen && <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
                    <motion.div
                      initial={{ scale: 0.9, opacity: 0 }}
                      animate={{ scale: 1, opacity: 1 }}
                      exit={{ scale: 0.9, opacity: 0 }}
                      transition={{ duration: 0.2 }}
                      className={`p-6 rounded-lg shadow-xl w-full max-w-sm ${mode === 'dark' ? 'bg-gray-800 text-white' : 'bg-white text-gray-900'
                        }`}
                    >
